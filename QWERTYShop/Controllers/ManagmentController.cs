...
namespace QWERTYShop.Controllers
{
    ...
    public class ManagmentController : Controller
    {
        ...
        private void RemoveImage(long id)
        {
            ...
            if (System.IO.File.Exists(path + $"{id}.jpg"))
            {
                System.IO.File.Delete(path + $"{id}.jpg");
            } else if (System.IO.File.Exists(path + $"{id}.jpeg"))
            {
                System.IO.File.Delete(path + $"{id}.jpeg");
            }
            else if (System.IO.File.Exists(path + $"{id}.png"))
            {
                System.IO.File.Delete(path + $"{id}.png");
            }
        ...
        public ActionResult Presentation(ManagmentSortModels model)
        {
            ...
            if (currentModel == "Remove last")
            {
                ...
                {
                    connection.Open();
                    NpgsqlCommand command = new NpgsqlCommand("select max(id) from cards", connection);
        ...
        private void SendEmail(string information, string mail)
        {
            ...
            client.Send("qwertysshopp@gmail.com", mail, "Заказ успешно оформлен!", information);
        }
        ...
        private void AddPropertiesToType(string properties, string type)
        {
            ...
            string checkCmd = $"select table_name from information_schema.tables where table_name='{specialName}'";
            using (var connection = new NpgsqlConnection(ConnectionString))
            {
                var cmd = new NpgsqlCommand(checkCmd, connection);
                connection.Open();
                ...
                while (reader.Read())
                {
                    if (reader.HasRows)
                    {
                        if (reader.GetString(0) == specialName)
                        {
            ...
            if (needToContinue)
            {
                ...
                CreateNewTable(propertiesArr, type, specialName);
            }
            else
            {
                ViewBag.Message = "Что-то пошло не так! Возможно Вы уже добавили свойства данному типу!";
            }
        ...
        private void CreateNewTable(string[] propertiesArr, string type, string specialName)
        {
            string body = "";
            ...
            current += body;
            current += $"constraint id_unique_{specialName} unique (id), ";
            current += $"constraint fk_{specialName} foreign key (id) references cards(id) on delete cascade);";
            using (NpgsqlConnection connection = new NpgsqlConnection(ConnectionString))
            {
                connection.Open();
